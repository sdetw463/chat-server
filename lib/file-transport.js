...
function wrapFile(filename, buffer) {
    ...
    if (name.length > 65535 || buffer.length > MAX_FILE_BYTES) throw new Error('文件超过传输包装上限（200MB）。');
    ...
    end.writeUInt32LE(local.length + name.length + buffer.length, 16);
...
module.exports = { prepareFileTransport, transportNote, unwrapFile, wrapFile };
