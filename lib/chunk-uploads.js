...
const UPLOAD_TTL_MS = 30 * 60000;
const UPLOAD_PREFIX = 'tuotuo-upload-';
const OWNER_FILE = '.upload-owner.json';
...
// A restart loses the in-memory upload map. Reclaim its abandoned directories,
// while never touching another running worker's upload or unrelated temp files.
async function pruneAbandonedUploads({ tempRoot = os.tmpdir(), now = Date.now(), processAlive = isProcessAlive } = {}) {
    const entries = await fs.readdir(tempRoot, { withFileTypes: true });
    for (const entry of entries) {
        if (!entry.isDirectory() || !/^tuotuo-upload-[A-Za-z0-9]+$/.test(entry.name)) continue;
        const dir = path.join(tempRoot, entry.name);
        try {
            let owner;
            try { owner = JSON.parse(await fs.readFile(path.join(dir, OWNER_FILE), 'utf8')); }
            catch (error) { if (error.code !== 'ENOENT' && !(error instanceof SyntaxError)) throw error; }
            if (owner?.hostname === os.hostname() && Number.isSafeInteger(owner.pid) && owner.pid > 0) {
                if (processAlive(owner.pid)) continue;
                // Only a directory with a valid local owner can be removed
                // immediately; legacy/foreign directories must first expire.
                await fs.rm(dir, { recursive: true, force: true });
                continue;
            }
            const directoryStat = await fs.stat(dir);
            const dataStat = await fs.stat(path.join(dir, 'data')).catch(error => {
                if (error.code === 'ENOENT') return null;
                throw error;
            });
            if (now - Math.max(directoryStat.mtimeMs, dataStat?.mtimeMs || 0) > UPLOAD_TTL_MS) {
                // Unknown owners may use a shared temp volume. Keep their
                // marked directories; a foreign host's PID cannot be checked.
                if (owner?.hostname && owner.hostname !== os.hostname()) continue;
                await fs.rm(dir, { recursive: true, force: true });
            }
...
function installChunkUploads(app, express, { identity, ready, active, persist, tempRoot = os.tmpdir() }) {
    const uploads = new Map();
    const startupCleanup = pruneAbandonedUploads({ tempRoot }).catch(error => {
        console.warn('清理上次进程的临时上传失败:', error.message);
    });
    ...
    const timer = setInterval(() => {
        for (const [id, item] of uploads) if (!item.busy && Date.now() - item.touched > UPLOAD_TTL_MS)
    ...
    app.post('/api/ai-chat/uploads', route(async (req, res) => {
        ...
        uploads.set(id, item);
        try {
            item.dir = await fs.mkdtemp(path.join(tempRoot, UPLOAD_PREFIX));
            await fs.writeFile(path.join(item.dir, OWNER_FILE), JSON.stringify({ pid: process.pid, hostname: os.hostname() }), { flag: 'wx' });
    ...
    return { close: async () => { clearInterval(timer); await startupCleanup; await Promise.all([...uploads].map(([id, item]) => remove(id, item))); } };
}
module.exports = { installChunkUploads, CHUNK_BYTES, pruneAbandonedUploads };
